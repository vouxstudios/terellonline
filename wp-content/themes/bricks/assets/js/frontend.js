...
const bricksUtils = {
	...
	/**
	 * Resolve the selector ID used to find query trail nodes in the DOM.
	 *
	 * @param {string} queryId Query element ID
	 * @param {string|boolean} componentId Component root ID
	 * @returns {string|boolean}
	 *
	 * @since 2.3.7 (#86c9z8rwk)
	 */
	getQueryTrailSelectorId: (queryId, componentId = false) => {
		if (componentId) {
			return componentId
		}

		return queryId && queryId.includes('-') ? queryId.split('-')[0] : queryId
	},
	...
	/**
	 * Update offcanvas toggle aria-label based on state
	 *
	 * @since 2.3.6
	 */
	setOffcanvasToggleAriaLabel: (toggle, isExpanded) => {
		if (!toggle) {
			return
		}

		const openLabel = toggle.dataset?.ariaLabelOpen || toggle.getAttribute('aria-label')
		const closeLabel = toggle.dataset?.ariaLabelClose || window.bricksData?.i18n?.close

		if (!openLabel || !closeLabel) {
			return
		}

		if (!toggle.dataset.ariaLabelOpen) {
			toggle.dataset.ariaLabelOpen = openLabel
		}

		toggle.setAttribute('aria-label', isExpanded ? closeLabel : openLabel)
	},
	...
	toggleAction: (toggle, customOptions = {}) => {
		...
		if (toggleElement.classList.contains('brxe-offcanvas')) {
			bricksUtils.setOffcanvasToggleAriaLabel(toggle, !expanded)
		}
	...
	/**
	 * Parse the Query Results Summary data without interrupting other query updates.
	 *
	 * Cached HTML may still contain the malformed attribute generated before #86caqzh4n.
	 *
	 * @param {Element} element
	 ...
	 * @since 2.3.10
	 */
	getQueryResultStatsData: (element) => {
		const statsData = element?.dataset?.brxQrStatsData || false
	...
	updateQueryResultStats: (queryId, type, data) => {
		...
		if (type === 'dom') {
			...
			if (queryResultsSummaryElement) {
				...
				const statsDataObj = bricksUtils.getQueryResultStatsData(queryResultsSummaryElement)
				if (statsDataObj) {
					...
					if (statsDataObj?.total !== undefined) {
						foundStats.count = statsDataObj.total
					}
				}
			}
		} else {
		...
		if (foundStats?.count !== undefined) {
			...
			queryResultsSummaryElement.forEach((stats) => {
				...
				const statsDataObj = bricksUtils.getQueryResultStatsData(stats)
				if (statsDataObj) {
					const statsFormat =
						typeof statsDataObj?.statsFormat === 'string' ? statsDataObj.statsFormat : ''
					const oneResultText =
						typeof statsDataObj?.oneResultText === 'string' ? statsDataObj.oneResultText : ''
					const noResultsText =
						typeof statsDataObj?.noResultsText === 'string' ? statsDataObj.noResultsText : ''
...
const bricksInitQueryLoopInstancesFn = new BricksFunction({
	...
	eachElement: (el) => {
		 ...
		 */
		const selectorId = bricksUtils.getQueryTrailSelectorId(queryElementId, componentId)
...
function bricksAjaxLoader() {
	...
	document.addEventListener('bricks/ajax/start', (event) => {
		...
		if (!targetElement) {
			const selectorId = bricksUtils.getQueryTrailSelectorId(
				queryId,
				queryLoopConfig?.componentId || false
			)

			// No result will cause the loader unable to detect the last element (@since 1.9.6)
			targetElement =
				Array.from(
					queryLoopConfig.resultsContainer?.querySelectorAll(`.brxe-${selectorId}:not(.brx-popup)`)
...
/**
 * Keep AJAX query updates understandable and navigable for assistive technology.
 *
 * Filter controls already restore their own focus after being re-rendered. Moving focus for every
 * filter change would interrupt that interaction, so only pagination moves focus to the first new
 * result. All successful pagination and filter updates are announced in a polite live region.
 *
 * @since 2.3.11
 */
function bricksQueryResultsA11y() {
	const busyQueries = new Map()
	const paginationQueries = new Set()
	const paginationCleanupTimers = new Map()
	const announcementTimers = new Map()

	/**
	 * Restore an attribute without overwriting a value supplied by the site author.
	 *
	 * @param {Element} element
	 * @param {string|null} value
	 */
	const restoreAriaBusy = (element, value) => {
		if (!element?.isConnected) {
			return
		}
	...
	/**
	 * Return the status element dedicated to a query instance.
	 *
	 * Separate live regions prevent simultaneous query updates from replacing each other's message.
	 *
	 * @param {string} queryId
	 * @returns {HTMLElement}
	 */
	const getLiveRegion = (queryId) => {
		const liveRegionId = `brx-query-results-status-${queryId}`
	...
	/**
	 * Build an announcement from the site's Query Results Summary when one is available.
	 *
	 * @param {string} queryId
	 * @param {boolean} isPagination
	 * @returns {string}
	 */
	const getAnnouncement = (queryId, isPagination) => {
		const updatedText = window.bricksData.i18n?.queryUpdated || ''
		...
		if (summaryText) {
			return `${updatedText} ${summaryText}`.trim()
		}

		const page = window.bricksData.queryLoopInstances[queryId]?.page || false
		const pageText = window.bricksData.i18n?.queryUpdatedPage || ''

		if (isPagination && page && pageText) {
			return pageText.replace('%s', page)
		}
	...
	/**
	 * Announce an update, clearing the region first so repeated messages are spoken again.
	 *
	 * @param {string} queryId
	 * @param {boolean} isPagination
	 */
	const announceUpdate = (queryId, isPagination) => {
		const message = getAnnouncement(queryId, isPagination)

		if (!message) {
			return
		}

		const liveRegion = getLiveRegion(queryId)
		clearTimeout(announcementTimers.get(queryId))
		...
		announcementTimers.set(
			...
			setTimeout(() => {
				...
				announcementTimers.delete(queryId)
			}, 50)
		)
	}

	/**
	 * Move pagination focus to the first result from the newly rendered page.
	 *
	 * @param {string} queryId
	 */
	const focusFirstResult = (queryId) => {
		...
		// Two frames let filter focus restoration and component initialization finish first.
		window.requestAnimationFrame(() => {
			window.requestAnimationFrame(() => {
				const selectorId = bricksUtils.getQueryTrailSelectorId(
					queryId,
					queryInstance.componentId || false
				)
				let focusTarget = resultsContainer.querySelector(
					`.brxe-${selectorId}:not(.brx-popup), .bricks-posts-nothing-found`
				)

				// An empty result set may only expose a Query Results Summary outside the loop container.
				if (!focusTarget) {
	...
	/**
	 * Finalize a successful result update.
	 *
	 * @param {string} queryId
	 * @param {boolean} isPagination
	 */
	const handleUpdate = (queryId, isPagination) => {
		if (!queryId) {
			return
		}

		announceUpdate(queryId, isPagination)

		if (isPagination) {
			focusFirstResult(queryId)
		}
	}

	// Remember pagination intent without coupling this behavior to either pagination request path.
	document.addEventListener('click', (event) => {
		const clickTarget = event.target || false

		// Delegated click targets can be non-Element nodes, which do not implement closest().
		if (!clickTarget || typeof clickTarget.closest !== 'function') {
			return
		}

		const paginationLink = clickTarget.closest('.brx-ajax-pagination a') || false

		if (!paginationLink || typeof paginationLink.closest !== 'function') {
			return
		}

		const pagination = paginationLink.closest('.brx-ajax-pagination') || false
		const queryId = pagination?.dataset?.queryElementId || false
		const queryInstance = window.bricksData.queryLoopInstances[queryId] || false

		// Missing query data aborts pagination before an AJAX event can clean up this intent.
		if (queryId && queryInstance?.resultsContainer) {
			clearTimeout(paginationCleanupTimers.get(queryId))
			paginationQueries.add(queryId)
		}
	})
	...
	document.addEventListener('bricks/ajax/start', (event) => {
		...
		const resultsContainer =
			window.bricksData.queryLoopInstances[queryId]?.resultsContainer || false

		if (!queryId || !resultsContainer) {
			return
		}

		const currentState = busyQueries.get(queryId)
		const requestVersion = (currentState?.requestVersion || 0) + 1

		busyQueries.set(queryId, {
			element: resultsContainer,
			previousValue: currentState
				? currentState.previousValue
				: resultsContainer.getAttribute('aria-busy'),
			requestVersion
		})
	...
	document.addEventListener('bricks/ajax/end', (event) => {
		...
		const busyState = busyQueries.get(queryId)

		// Promise callbacks render filter results immediately after ajax/end; defer cleanup until then.
		if (busyState) {
			setTimeout(() => {
				const queryInstance = window.bricksData.queryLoopInstances[queryId] || false

				// A newer filter request can still be loading when an older request finishes.
				if (
					!queryInstance?.isLoading &&
					busyQueries.get(queryId)?.requestVersion === busyState.requestVersion
				) {
					restoreAriaBusy(busyState.element, busyState.previousValue)
					busyQueries.delete(queryId)
				}
			}, 0)
		}

		// A failed filter request has no displayed event, so discard stale pagination intent here.
		paginationCleanupTimers.set(
			queryId,
			setTimeout(() => {
				paginationQueries.delete(queryId)
				paginationCleanupTimers.delete(queryId)
			}, 0)
		)
	})
	...
	document.addEventListener('bricks/ajax/query_result/displayed', (event) => {
		const queryId = event.detail?.queryId || false
		const isPagination = paginationQueries.has(queryId)

		clearTimeout(paginationCleanupTimers.get(queryId))
		paginationCleanupTimers.delete(queryId)
		paginationQueries.delete(queryId)
		handleUpdate(queryId, isPagination)
	})
	...
	document.addEventListener('bricks/ajax/pagination/completed', (event) => {
		...
		// The public completion event also fires for failed requests for backwards compatibility.
		if (event.detail?.queryUpdated) {
			handleUpdate(queryId, true)
		}
	})
}
...
function bricksQueryLoadPage(el, noDelay = false, nonceRefreshed = false) {
	return new Promise(function (resolve, reject) {
		...
		let queryData = {
			...
			urlParams: bricksGetCurrentUrlParams() // #86c21d4m0; @since 2.3.8
		}
			...
			if (xhr.readyState === XMLHttpRequest.DONE) {
				...
				setTimeout(() => {
					...
					let newQueryTrail = Array.from(
						queryInfo.resultsContainer?.querySelectorAll(`.brxe-${selectorId}:not(.brx-popup)`) ||
							[]
					).pop()
		...
		xhr.onreadystatechange = function () {
			if (this.readyState === XMLHttpRequest.DONE) {
				...
				if (status === 0 || (status >= 200 && status < 400)) {
					...
					const selectorId = bricksUtils.getQueryTrailSelectorId(queryId, queryComponentId)
						...
						let player = new YT.Player(videoElement, {
							...
							events: {
								onReady: function (event) {
									videoElement = event.target.getIframe()
...
const bricksPhotoswipeFn = new BricksFunction({
	...
	eachElement: (lightboxElement) => {
		...
		let options = {
			...
			closeSVG: closeSVG,
			closeTitle: window.bricksData.i18n.close,
			zoomTitle: window.bricksData.i18n.zoom,
			arrowPrevTitle: window.bricksData.i18n.prevSlide,
			arrowNextTitle: window.bricksData.i18n.nextSlide,
		...
		const hasMismatchedLightboxSource = (itemData) => {
			return !!itemData?.src && !!itemData?.msrc && itemData.src !== itemData.msrc
		}
		...
		// Prevent a visible source swap when the clicked image differs from the lightbox source.
		lightbox.addFilter('placeholderSrc', (placeholderSrc, content) => {
			return placeholderSrc && hasMismatchedLightboxSource(content?.data)
				? content.data.src
				: placeholderSrc
		})

		// Crossfade back to the page thumbnail instead of revealing a final-frame source swap.
		lightbox.on('closingAnimationStart', () => {
			const currentSlide = lightbox.pswp?.currSlide
			const closeDuration = lightbox.pswp?.options?.hideAnimationDuration

			if (!hasMismatchedLightboxSource(currentSlide?.data) || !closeDuration) {
				return
			}

			const easing = lightbox.pswp?.options?.easing || 'cubic-bezier(.4,0,.22,1)'
			const fadeElements = [
				currentSlide?.content?.element,
				currentSlide?.content?.placeholder?.element
			].filter(Boolean)

			fadeElements.forEach((element) => {
				element.style.transition = `opacity ${closeDuration}ms ${easing}`
	...
	eachElement: (toc) => {
		...
		function initializeTocbot(toc) {
			...
			let options = {
				...
				// Use document-level heading offsets when headings sit inside positioned wrappers (#86c02fu18; @since 2.3.6)
...
const bricksFormFn = new BricksFunction({
	...
	eachElement: (form) => {
		...
		/**
		 * Validate email syntax and local-part dot placement.
		 *
		 * @param {string} value
		 * @returns {boolean}
		 */
		function isValidEmail(value) {
			...
			const hasInvalidLocalPartDots = /^\.|\.$|\.\./.test(localPart)

			return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) && !hasInvalidLocalPartDots
		}
		...
		function validateFileInput(fileInput, files) {
			...
			if (fileInput.hasAttribute('required') || fileInput.dataset.required === 'true') {
				...
				const hasExistingFiles = formGroup.querySelector(
					'input[name^="bricks_form_existing_files"]'
				)

				// Show error if no files are selected in both input and files object
				if (!hasFilesInInput && !hasFilesInObject && !hasExistingFiles) {
					showError = true
				}
			}
		...
		fileInputInstances.forEach((input) => {
			...
			input.addEventListener('change', (e) => {
				...
				if (maxLength) {
					existingFileCount = form.querySelectorAll(
						`.file-result.existing-file[data-files-ref="${inputRef}"]`
					).length
				}
				...
				for (let i = 0; i < fileListLength; i++) {
					...
					let fileCount = files.hasOwnProperty(inputName) ? files[inputName].length : 0

					if (maxLength && fileCount + existingFileCount >= maxLength) {
						error = 'limit'
					}
				...
				input.files = validFileList.files
			})
		})

		let existingFileResults = bricksQuerySelectorAll(form, '.file-result.existing-file')

		existingFileResults.forEach((resultEl) => {
			let removeButton = resultEl.querySelector('.remove')
			...
			removeButton.addEventListener('click', () => {
				let fieldId = removeButton.getAttribute('data-field')
				let fileValue = removeButton.getAttribute('data-existing-file-value')
				let removedInput = document.createElement('input')

				removedInput.type = 'hidden'
				removedInput.name = `bricks_form_removed_files[${fieldId}][]`
				removedInput.value = fileValue

				form.appendChild(removedInput)
				resultEl.remove()

				let inputRef = resultEl.getAttribute('data-files-ref')
				let fileInput = form.querySelector(`input[type=file][data-files-ref="${inputRef}"]`)

				if (
					fileInput &&
					fileInput.dataset.required === 'true' &&
					!form.querySelector(`.file-result.existing-file[data-files-ref="${inputRef}"]`)
				) {
				...
				if (
					fileInput &&
					fileInput.getAttribute('data-error-message') &&
					!(validationDisabledOn.includes('input') && validationDisabledOn.includes('blur')) &&
					bricksIsFrontend
				) {
					validateFileInput(fileInput, files)
				}
			})
		})
...
function bricksSubmitForm(elementId, form, files, recaptchaToken, nonceRefreshed) {
	...
	/**
	 * Preserve the signed ACF query chain for server-side dynamic data rendering.
	 * Forms outside ACF row loops and older cached markup omit these attributes and
	 * continue through the existing submission path.
	 *
	 * #86cazz5mc; @since 2.3.11
	 */
	if (form.dataset.formQueryContext && form.dataset.formQueryNonce) {
		formData.append('formQueryContext', form.dataset.formQueryContext)
		formData.append('formQueryNonce', form.dataset.formQueryNonce)
	}
...
const bricksSplideFn = new BricksFunction({
	...
	eachElement: (splideElement) => {
		...
		// Init & mount splideJS
		let splideInstance = new Splide(splideElement, splideOptions)
...
/**
 * Refresh Splide when hidden containers become visible.
 *
 * Keep this as one document listener instead of registering one listener per slider instance.
 */
function bricksSplideListeners() {
	const refreshSplideInstances = (element) => {
		if (!element) {
			return
		}

		// Limit refreshes to sliders inside the tab pane or accordion item that changed.
		let splideElements = element.matches?.('.brxe-slider-nested.splide') ? [element] : []
		splideElements = [...splideElements, ...element.querySelectorAll('.brxe-slider-nested.splide')]

		splideElements.forEach((splideElement) => {
			let scriptId = splideElement?.dataset?.scriptId
			let splideInstance = scriptId ? window.bricksData?.splideInstances?.[scriptId] : false

			if (!splideInstance) {
				return
			}

			// Splide can clone a playing preview-image iframe during refresh. Keep the active iframe intact.
			...
			// Let the browser apply the visibility/layout change before Splide reads dimensions.
			window.requestAnimationFrame(() => {
				if (splideElement.isConnected) {
	...
	document.addEventListener('bricks/tabs/changed', (event) => {
		refreshSplideInstances(event.detail?.activePane)
	})

	document.addEventListener('bricks/accordion/open', (event) => {
		refreshSplideInstances(event.detail?.openItem)
	})
}
...
// Recheck viewport-based Swiper breakpoints when the slider container itself no longer resizes.
function bricksSwiperUpdateBreakpoints() {
	Object.keys(window.bricksData?.swiperInstances || {}).forEach((scriptId) => {
		const swiperInstance = window.bricksData.swiperInstances[scriptId]
...
function bricksInteractionCallbackExecution(sourceEl, config) {
	...
	switch (config?.action) {
		...
		case 'loadMore':
			...
			const selectorId = bricksUtils.getQueryTrailSelectorId(queryId, componentId)
	...
	const offcanvasAction = (offcanvas) => {
			...
			if (offcanvasToggles.length) {
				offcanvasToggles.forEach((offcanvasToggle) => {
					...
					if (isTargetCurrentOffcanvas) {
						...
						bricksUtils.setOffcanvasToggleAriaLabel(offcanvasToggle, true)
					}
				})
			}
		}
		...
		// STEP: Close offcanvas
			...
			if (toggleNode) {
				toggleNode.setAttribute('aria-expanded', false)
				toggleNode.classList.remove('is-active')
				bricksUtils.setOffcanvasToggleAriaLabel(toggleNode, false)
				toggleNode.focus()
			}
	...
	openOffcanvasElements.forEach((openOffcanvas) => {
		...
		if (offcanvasToggles.length) {
			offcanvasToggles.forEach((offcanvasToggle) => {
				...
				bricksUtils.setOffcanvasToggleAriaLabel(offcanvasToggle, false)
			})
		}
	})
}
...
const bricksSubmenuListenersFn = new BricksFunction({
	...
	eachElement: (submenuItem) => {
		...
		// Ignore false boundary events when the pointer remains within this item,
		// which can occur when embedded widgets such as hCaptcha update the browser's hit testing.
		const isInternalMouseTransition = (e) =>
			e.relatedTarget instanceof Node && submenuItem.contains(e.relatedTarget)

		// Open submenu on mouseenter
		submenuItem.addEventListener('mouseenter', function (e) {
			// Return: Browser reported a boundary change within the same menu item
			if (isInternalMouseTransition(e)) {
				return
			}
			...
			let toggle = submenuItem.querySelector(
				':scope > .brx-submenu-toggle > [aria-expanded="false"]'
			)
		...
		submenuItem.addEventListener('mouseleave', function (e) {
			...
			let toggle = submenuItem.querySelector(
				':scope > .brx-submenu-toggle > [aria-expanded="true"]'
			)
...
function bricksGetVisibleFocusables(node) {
	// Rendered focus targets can have a zero-sized box when visually hidden for accessibility (#86capxhf5 @since 2.3.10)
...
function bricksDisplayQueryResult(targetQueryId, res) {
	...
	const queryComponentId = queryInstance?.componentId || false
	const selectorId = bricksUtils.getQueryTrailSelectorId(targetQueryId, queryComponentId)
...
document.addEventListener('DOMContentLoaded', (event) => {
	...
	window.addEventListener('resize', () => {
		...
		if (bricksIsFrontend) {
			...
			// Keep Swiper breakpoint settings in sync after capped containers stop triggering resize observers.
			bricksTimeouts.bricksSwiperBreakpoints = setTimeout(bricksSwiperUpdateBreakpoints, 250)
		}
